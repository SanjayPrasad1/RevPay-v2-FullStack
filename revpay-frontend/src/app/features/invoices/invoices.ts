...
export class InvoicesComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // this.accountType = this.authService.currentUser;
    ...
    if (this.accountType === 'BUSINESS') {
      this.load(); // Logic for Business: Load invoices you CREATED
    } else {
      this.loadReceived(); // Logic for Personal: Load invoices you RECEIVED
    }
    }
  }
  ...
  // In invoices.component.ts

pay(invoice: any) {
  if (confirm(`Are you sure you want to pay $${invoice.totalAmount} to ${invoice.businessName}?`)) {
    this.invoiceApi.payInvoice(invoice.id).subscribe({
      next: (res) => {
        this.snackBar.open('Invoice paid successfully!', 'Close', { duration: 3000 });
        this.loadReceived(); // Refresh the "Received Invoices" list
      },
      error: (err) => {
        this.snackBar.open(err.error.message || 'Payment failed', 'Close');
      }
    });
  }
}
